...
if [ $machine == "x86_64" ];then
  ...
  if [ ! -f mysql-5.6.21-linux-glibc2.5-x86_64.tar.gz ];then
	  wget http://res1.coder4j.com/onekey/mysql/mysql-5.6.21-linux-glibc2.5-x86_64.tar.gz
  fi
  tar -xzvf mysql-5.6.21-linux-glibc2.5-x86_64.tar.gz
  mv mysql-5.6.21-linux-glibc2.5-x86_64/* /lnamp/server/mysql
else
  rm -rf mysql-5.6.21-linux-glibc2.5-i686
  if [ ! -f mysql-5.6.21-linux-glibc2.5-i686.tar.gz ];then
    wget http://res1.coder4j.com/onekey/mysql/mysql-5.6.21-linux-glibc2.5-i686.tar.gz
  fi
  tar -xzvf mysql-5.6.21-linux-glibc2.5-i686.tar.gz
  mv mysql-5.6.21-linux-glibc2.5-i686/* /lnamp/server/mysql
fi
...
log-error=/lnamp/log/mysql/error.log
key_buffer_size = 16M
