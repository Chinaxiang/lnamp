...
if [ ! -f php-5.3.29.tar.gz ];then
  wget http://res1.coder4j.com/onekey/php/php-5.3.29.tar.gz
fi
...
make install
cd ..
cp ./php-5.3.29/php.ini-production /lnamp/server/php/etc/php.ini
#adjust php.ini
sed -i 's#; extension_dir = \"\.\/\"#extension_dir = "/lnamp/server/php/lib/php/extensions/no-debug-non-zts-20090626/"#'  /lnamp/server/php/etc/php.ini
sed -i 's#extension_dir = \"\.\/\"#extension_dir = "/lnamp/server/php/lib/php/extensions/no-debug-non-zts-20090626/"#'  /lnamp/server/php/etc/php.ini
