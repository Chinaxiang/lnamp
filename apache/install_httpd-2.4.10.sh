...
if [ ! -f httpd-2.4.10.tar.gz ];then
  wget http://res1.coder4j.com/onekey/httpd/httpd-2.4.10.tar.gz
fi
tar -zxvf httpd-2.4.10.tar.gz

if [ ! -f apr-1.5.0.tar.gz ];then
  wget http://res1.coder4j.com/onekey/apache/apr-1.5.0.tar.gz
fi
tar -zxvf apr-1.5.0.tar.gz
cp -rf apr-1.5.0 httpd-2.4.10/srclib/apr

if [ ! -f apr-util-1.5.3.tar.gz ];then
  wget http://res1.coder4j.com/onekey/apache/apr-util-1.5.3.tar.gz
fi
...
sed -i 's/MaxRequestWorkers      150/MaxRequestWorkers      255/g' /lnamp/server/httpd/conf/extra/httpd-mpm.conf
